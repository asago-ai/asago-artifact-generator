"""Build examples/demo/demo.ipynb. Run from repo root or this directory."""
...
`asago_artifact_generator` is the translator:
...
The notebook locates the **agent-policy-redteam** repo root even if you launched Jupyter from `examples/demo/`, `src/asago_artifact_generator/garak/`, or the repo root. It also records the sibling Garak checkout (override with `GARAK_ROOT` if yours lives elsewhere).
"""),
    code("""\
from __future__ import annotations
...
for candidate in (HERE, *HERE.parents):
    if (candidate / "src" / "asago_artifact_generator" / "garak" / "gen.py").is_file():
        if str(candidate) not in sys.path:
            sys.path.insert(0, str(candidate))
        break

try:
    from examples.demo.demo_runtime import (
    ...
    demo_dir = HERE if (HERE / "demo_runtime.py").is_file() else HERE
...
`seed_id` prefixes map to surfaces (see `src/asago_artifact_generator/garak/classify.py`). Unknown seeds default to `user_turn`. `AP-T17-02` is `tool_definition` and will **skip** generation.
...
"""),
    code("""\
from asago_artifact_generator.garak.spec_io import garak_artifact_path
...
```bash
python -m asago_artifact_generator.cli generate examples/scenarios/<id>.yaml --force -v
```
...
"""),
    code("""\
from asago_artifact_generator.garak.spec_io import garak_artifact_path, validation_path
...
1. Copies `src/asago_artifact_generator/garak/plugins/probes/scenario.py` and `.../detectors/scenario.py` into your Garak checkout so `--probes scenario.Scenario` resolves.
...
```bash
python -m asago_artifact_generator.cli generate examples/scenarios/AP-T2-01-28712e.yaml --force -v
# then this notebook's Garak cell, or:
python -m asago_artifact_generator.cli generate --artifact runs/AP-T2-01-28712e/AP-T2-01-28712e-garak.json
```
